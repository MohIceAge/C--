  ...
  and compile_code  = function
    ...
    | CRETURN(Some((_, expr))) -> (compile_expr expr)^"\tmovq %rbp,%rsp\n\tpopq %rbp\n\tret\n"
      | CTHROW(name, (_, expr)) -> (match !tryStack with
          [] -> "#return + exception"
        | id::tryStack2 -> tryStack := tryStack2; (compile_expr (STRING(name)))^"\tpushq %rax\n"^(compile_expr expr)^"\tjmp .try"^id^"_b\n")
      ...
      let id = uniqid () in tryStack:=id::!tryStack; joinLines ([
        ...
        List.map (fun a -> let (nam, var, loc_code) = a in let (loc, code) = loc_code in 
          ...
          let exceptId = ".except"^id^"_"^nam in
            exceptId^"_b:\n"^
            "\tsubq $8, %rsp \n"^
            "\tmovq %rax, -"^(string_of_int ((List.length !local_vars)*8))^"(%rbp)\n"^
            "\tpopq %rdi\n"^
            (compile_expr (STRING(nam)))^
            "\tmovq %rax, %rsi\n"^
            "\tcall strcmp\n"^
            "\tcmpq $0, %rax\n"^
            "\tjne "^exceptId^"_e\n"^
      ...
      )@( (match fin with None -> "" | Some((_, code)) -> compile_code code)::["\n.try"^id^"_e:"]
      ))

  and compile_expr = function
      CST(i) -> Printf.sprintf "\tmovq $%d, %%rax\n" i
    | VAR(s) -> Printf.sprintf "\tmovq %s, %%rax \n" (var_reg s)
    | STRING(s) -> let i = match find_i s !strings with None -> strings := s::!strings; 0 | Some(i) -> i in
                 Printf.sprintf "\tleaq .ST%d(%%rip), %%rax\n" (i+1)
